    ...
    var service = {
      ...
      getCrypto: function(){
        if(!cryptoData){
          ...
          $http.get('http://localhost:9000/api/coinmarketdata', { cache: true }).then(function(resp) {
  ...
  angular.module('BlurAdmin.pages.main').service('ComparisonService', function($http, $q) {
    ...
    var service = {
        getComparisonData: function(){
            if(!cryptoData){
                var deferred = $q.defer();
                $http.get('http://localhost:9000/api/comparison', { cache: true }).then(function(resp) {
                //$http.get('https://api.coinmarketcap.com/v1/ticker/', { cache: true }).then(function(resp) {
                    cryptoData =  resp.data;
                    deferred.resolve(cryptoData);
                }, function(error) {
